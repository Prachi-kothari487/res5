...
const StaffDashboard = () => {
  ...
  // ✅ These must match backend statuses
  const statusOptions = ["placed", "preparing", "served", "cancelled"];

  // ✅ Fetch orders based on current status
  const fetchOrders = async () => {
    try {
      setLoading(true);
      const token = localStorage.getItem("token");
      ...
      const res = await axios.get(
        `http://10.137.209.69:4000/api/orders/status/${filterStatus}`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  const handleStatusUpdate = async (orderId, newStatus) => {
    try {
      const token = localStorage.getItem("token");
      await axios.patch(
        `http://10.137.209.69:4000/api/orders/${orderId}/status`,
        { status: newStatus },
        { headers: { Authorization: `Bearer ${token}` } }
      );
      fetchOrders();
  ...
  return (
    ...
    >
            ...
            style={{
              ...
              border:
                filterStatus === status
                  ...
                  : "1px solid #e5d8a1",
              backgroundColor:
                filterStatus === status ? "#b8860b" : "#fffaf0",
              color: filterStatus === status ? "#fff" : "#8b7500",
              cursor: "pointer",
              fontSize: "14px",
              fontWeight: "500",
      ...
      {/* Loading / Error */}
      {loading && (
        <p style={{ textAlign: "center", color: "#8b7500" }}>
          Loading orders...
        </p>
      )}
      {error && <p style={{ textAlign: "center", color: "red" }}>{error}</p>}
